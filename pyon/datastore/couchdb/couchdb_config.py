...
function(doc) {
  if (doc.type_ == "DirEntry") {
    for (var attr in doc.attributes) {
      if (typeof(doc.attributes[attr]) == "string" && doc.attributes[attr].length > 1500)      {
         doc_attr = doc.attributes[attr].substring(0,1500);
         emit([doc.org, attr, doc_attr, doc.parent], doc);
      }
      else{
